...
class AgentStreamExecutor:
    ...
    def __init__(
        ...
        logger.info(f"[AgentStreamExecutor] model type: {type(model)}, has call_stream: {hasattr(model, 'call_stream')}")
    ...
    def run_stream(self, user_message: str) -> str:
        ...
        try:
            while turn < self.max_turns:
                ...
                try:
                    for tool_call in tool_calls:
                        ...
                        # ⚡ 短路检测：bash 工具输出 MUSIC_CARD: 时跳过第2轮 LLM
                        # 注：search_music.py 的 card 子命令仍输出 MUSIC_CARD:，
                        #     random/detail/play 已改为输出简洁 JSON（由 agent_bridge 处理 MUSIC_PLAY:）
                        # 用 find 检测，兼容输出前有其他内容的情况
                        _MUSIC_CARD_PREFIX = "MUSIC_CARD:"
                        if (
                            result.get("status") == "success"
                            and tool_call.get("name") == "bash"
                            and isinstance(result_content, str)
                        ):
                            _mc_pos = result_content.find(_MUSIC_CARD_PREFIX)
                            if _mc_pos >= 0:
                                _mc_xml = result_content[_mc_pos + len(_MUSIC_CARD_PREFIX):].strip()
                                if _mc_xml:
                                    logger.info(
                                        f"⚡ [AgentStream] 检测到 MUSIC_CARD 输出，短路返回，"
                                        f"跳过第2轮LLM (xml长度={len(_mc_xml)})"
                                    )
                                    final_response = f"MUSIC_CARD:{_mc_xml}"
                                    # 注：finally 块负责写入 tool_result_blocks 到消息历史
                                    return final_response

                        # ⚡ 短路检测：bash 工具输出 IMAGE_URL: 时跳过第2轮 LLM
                        # 注：bizyair-nano-banana 等图像生成 Skill 输出 IMAGE_URL: 标记
                        # 框架层会直接解析并发送图片，无需 LLM 再处理
                        _IMAGE_URL_PREFIX = "IMAGE_URL:"
                        if (
                            result.get("status") == "success"
                            and tool_call.get("name") == "bash"
                            and isinstance(result_content, str)
                        ):
                            # 处理可能的 JSON 转义（result_content 可能是序列化后的 JSON）
                            # 先尝试从原始 result.output 中获取（未转义的原始输出）
                            _raw_output = result.get("result", "")
                            if isinstance(_raw_output, str):
                                _search_content = _raw_output
                            else:
                                _search_content = result_content
                            
                            _iu_pos = _search_content.find(_IMAGE_URL_PREFIX)
                            if _iu_pos >= 0:
                                # 提取 URL，使用正则表达式精确匹配 https://... 到 .png/.jpg/.jpeg/.webp
                                import re
                                _after_prefix = _search_content[_iu_pos + len(_IMAGE_URL_PREFIX):]
                                # 匹配 URL 模式：http(s)://... 直到文件扩展名或空白/换行
                                _url_match = re.search(r'https?://[^\s\\"\'\n\r]+', _after_prefix)
                                if _url_match:
                                    _img_url = _url_match.group(0)
                                    # 去除可能的尾随标点
                                    _img_url = _img_url.rstrip('.,;:!?')
                                    
                                    if _img_url.startswith(("http://", "https://")):
                                        logger.info(
                                            f"⚡ [AgentStream] 检测到 IMAGE_URL 输出，短路返回，"
                                            f"跳过第2轮LLM (url={_img_url[:80]}...)"
                                        )
                                        final_response = f"IMAGE_URL:{_img_url}"
                                        # 注：finally 块负责写入 tool_result_blocks 到消息历史
                                        return final_response
                    ...
                    # 同样适用于 MUSIC_CARD 短路场景（Python 在 return 后也会执行 finally）
                    if tool_result_blocks:
                        ...
                        # 跳过 MUSIC_CARD/IMAGE_URL 短路场景，避免写入多余的提示消息到历史
                        if (
                            not isinstance(final_response, str)
                            or not (final_response.startswith("MUSIC_CARD:") or final_response.startswith("IMAGE_URL:"))
                        ) and turn >= 3 and len(tool_calls) > 0:
            ...
            err_str = str(e)
            logger.error(f"❌ Agent执行错误: {err_str}")
            self._emit_event("error", {"error": err_str})

            # 将具体错误信息返回给用户，不再使用通用 fallback
            if '[api_error]' in err_str.lower():
                # 从 [API_ERROR] 标记后提取裸错误信息
                # 简化 HTTP 5xx 错误，提取核心 message 字段
                import re as _re, json as _json
                raw = err_str.replace('[API_ERROR] ', '').replace('[api_error] ', '')
                # 尝试从 JSON 错误体里提取 message 字段
                _m = _re.search(r'\{.*\}', raw, _re.DOTALL)
                if _m:
                    try:
                        _body = _json.loads(_m.group())
                        _inner = _body.get('error', {})
                        _msg = _inner.get('message', '') if isinstance(_inner, dict) else ''
                        # 只取 Upstream 500 之后的核心描述，去掉 HTML
                        if _msg:
                            _msg = _re.sub(r'<[^>]+>', '', _msg).strip()  # strip HTML
                            _msg = _re.sub(r'\s+', ' ', _msg).strip()
                        raw = _msg or raw
                    except Exception:
                        pass
                final_response = f"⚠️ API 调用失败：{raw[:200]}"
            elif '[context_overflow]' in err_str.lower():
                final_response = "⚠️ 对话历史过长，已自动清空，请重新发送消息。"
            else:
                final_response = f"⚠️ 出现错误：{err_str[:200]}"
                ...
                if isinstance(chunk, dict) and chunk.get("choices"):
                    ...
                    if "tool_calls" in delta and delta["tool_calls"]:
                        for tc_delta in delta["tool_calls"]:
                            ...
                            if "id" in tc_delta:
                                # ✅ 防护 None：部分模型首包 id 为 None
                                tool_calls_buffer[index]["id"] = tc_delta["id"] or ""

                            if "function" in tc_delta:
                                func = tc_delta["function"]
                                if "name" in func:
                                    # ✅ 防护 None：name 在后续增量 chunk 中可能为 None
                                    tool_calls_buffer[index]["name"] += (func["name"] or "")
                                if "arguments" in func:
                                    # ✅ 防护 None：首个 chunk 的 arguments 可能为 None
                                    tool_calls_buffer[index]["arguments"] += (func["arguments"] or "")
